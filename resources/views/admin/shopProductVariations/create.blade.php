...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.shopProductVariation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shop-product-variations.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop_product') ? 'is-invalid' : '' }}" name="shop_product_id" id="shop_product_id" required>
                    @foreach($shop_products as $id => $entry)
                        <option value="{{ $id }}" {{ old('shop_product_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
