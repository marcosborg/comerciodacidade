...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.myShop.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.my-shops.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop_categories') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ in_array($id, old('shop_categories', [])) ? 'selected' : '' }}>{{
                        $shop_category }}</option>
                    @endforeach
                </select>
