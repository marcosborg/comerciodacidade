...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.shopSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shop-schedules.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_employee') ? 'is-invalid' : '' }}" name="service_employee_id" id="service_employee_id" required>
                    @foreach($service_employees as $id => $entry)
                        <option value="{{ $id }}" {{ old('service_employee_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
